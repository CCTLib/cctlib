...
#define HANDLE_CASE(NUM)                                                                                                                                                                                 \
    case (NUM): {                                                                                                                                                                                        \
        if (INS_MemoryOperandIsRead(ins, memOp)) {                                                                                                                                                       \
            INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)MemAnalysis<(NUM)>::RecordNByteMemRead, IARG_MEMORYOP_EA, memOp, IARG_UINT32, opaqueHandle, IARG_INST_PTR, IARG_THREAD_ID, IARG_END);  \
        } else {                                                                                                                                                                                         \
            INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)MemAnalysis<(NUM)>::RecordNByteMemWrite, IARG_MEMORYOP_EA, memOp, IARG_UINT32, opaqueHandle, IARG_INST_PTR, IARG_THREAD_ID, IARG_END); \
        }                                                                                                                                                                                                \
    } break
