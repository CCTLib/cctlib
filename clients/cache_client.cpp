...
struct Cache_t {
    ...
    Cache_t()
        : tag(0), isDirty(false), isInUse(false), wasDifferent(false) {}
};
...
struct Stats_t {
    ...
    Stats_t()
        : sameData(0), evicts(0), unchanged(0), dirtyEvicts(0) {}
};
...
struct RedSpyAnalysis {
    ...
    static __attribute__((always_inline)) bool IsWriteRedundant(void*& addr, THREADID threadId) {
        ...
        switch (AccessLen) {
        case 1:
            return *((uint8_t*)(&avPair->value)) == *(static_cast<uint8_t*>(avPair->address));
        case 2:
            return *((uint16_t*)(&avPair->value)) == *(static_cast<uint16_t*>(avPair->address));
        case 4:
            return *((uint32_t*)(&avPair->value)) == *(static_cast<uint32_t*>(avPair->address));
        case 8:
            return *((uint64_t*)(&avPair->value)) == *(static_cast<uint64_t*>(avPair->address));
        default:
            return memcmp(&avPair->value, avPair->address, AccessLen) == 0;
        }
    }
    static __attribute__((always_inline)) VOID RecordNByteValueBeforeWrite(void* addr, THREADID threadId) {
        ...
        switch (AccessLen) {
        case 1:
            *((uint8_t*)(&avPair->value)) = *(static_cast<uint8_t*>(addr));
            break;
        case 2:
            *((uint16_t*)(&avPair->value)) = *(static_cast<uint16_t*>(addr));
            break;
        case 4:
            *((uint32_t*)(&avPair->value)) = *(static_cast<uint32_t*>(addr));
            break;
        case 8:
            *((uint64_t*)(&avPair->value)) = *(static_cast<uint64_t*>(addr));
            break;
        default:
...
#define HANDLE_CASE(NUM, BUFFER_INDEX, HAS_FALLTHRU)                                                                                                                                                          \
    case (NUM): {                                                                                                                                                                                             \
        ...
        INS_InsertPredicatedCall(ins, HAS_FALLTHRU ? IPOINT_AFTER : IPOINT_TAKEN_BRANCH, (AFUNPTR)RedSpyAnalysis<(NUM), (BUFFER_INDEX)>::CheckNByteValueAfterWrite, IARG_THREAD_ID, IARG_INST_PTR, IARG_END); \
    } break
