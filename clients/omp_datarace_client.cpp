...
class Label {
  ...
  public:
    ...
    explicit Label(LabelCreationType type, const Label& label, const LabelSegment& extension = defaultExtension) {
        switch (type) {
        case CREATE_AFTER_FORK:
            LabelCreateAfterFork(label, extension);
            break;
        case CREATE_AFTER_JOIN:
            LabelCreateAfterJoin(label);
            break;
        case CREATE_AFTER_BARRIER:
            LabelCreateAfterBarrier(label);
            break;
        case CREATE_AFTER_ENTERING_ORDERED_SECTION:
            LabelCreateAfterEneringOrderedSection(label);
            break;
        case CREATE_AFTER_EXITING_ORDERED_SECTION:
            LabelCreateAfterExitingOrderedSection(label);
            break;
        default:
            assert(false);
        }
...
static inline VOID CheckRace(VOID* addr, uint32_t accessLen, bool accessType, uint32_t opaqueHandle, THREADID threadId) {
    ...
    if (overflow <= 0) {
        ...
        status = get<0>(sm.GetOrCreateShadowBaseAddress((size_t)(((char*)addr) + accessLen))); // +accessLen so that we get next page
