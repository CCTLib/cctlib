    ...
    static __attribute__((always_inline)) void CheckValues(PIN_REGISTER* regRef, uint8_t regInd, uint32_t opaqueHandle, THREADID threadId) {
        ...
        if (simdType == 0) {
            ...
            if (sizeof(T) == 4) {
                ...
                if (rate[1] <= delta && rate[1] >= -delta)
                    redCount++;
                if (rate[2] <= delta && rate[2] >= -delta)
                    redCount++;
                if (rate[3] <= delta && rate[3] >= -delta)
                    redCount++;
...
static inline uint32_t GetAliasIDs(REG reg) {
    ...
    case LEVEL_BASE::REG_RAX:
        regGroup = ALIAS_REG_A;
        byteInd = ALIAS_BYTES_INDEX_64;
        type = ALIAS_GENERIC;
        break;
    case REG_EAX:
        regGroup = ALIAS_REG_A;
        byteInd = ALIAS_BYTES_INDEX_32;
        type = ALIAS_GENERIC;
        break;
    case REG_AX:
        regGroup = ALIAS_REG_A;
        byteInd = ALIAS_BYTES_INDEX_16;
        type = ALIAS_GENERIC;
        break;
    case REG_AH:
        regGroup = ALIAS_REG_A;
        byteInd = ALIAS_BYTES_INDEX_8_H;
        type = ALIAS_HIGH_BYTE;
        break;
    case REG_AL:
        regGroup = ALIAS_REG_A;
        byteInd = ALIAS_BYTES_INDEX_8_L;
        type = ALIAS_LOW_BYTE;
        break;

    case LEVEL_BASE::REG_RBX:
        regGroup = ALIAS_REG_B;
        byteInd = ALIAS_BYTES_INDEX_64;
        type = ALIAS_GENERIC;
        break;
    case REG_EBX:
    ...
    default:
        assert(0 && "not alias registers! should not reach here!");
        break;
    }
...
static inline void InstrumentAliasReg(INS ins, REG reg, uint16_t oper, uint32_t opaqueHandle) {
    ...
    if (IsFloatInstruction(INS_Address(ins))) {
        ...
        case 2:
        case 4:
            HANDLE_APPROXREG(float, true, regId);
            break;
        case 8:
            HANDLE_APPROXREG(double, true, regId);
            break;
        default:
            break;
        }
        ...
        switch (regSize) {
        case 8:
            HANDLE_ALIAS_REG(uint64_t, ALIAS_GENERIC, regId);
            break;
        case 4:
            HANDLE_ALIAS_REG(uint32_t, ALIAS_GENERIC, regId);
            break;
        case 2:
            HANDLE_ALIAS_REG(uint16_t, ALIAS_GENERIC, regId);
            break;
        case 1:
...
static inline void InstrumentGeneralReg(INS ins, REG reg, uint16_t oper, uint32_t opaqueHandle) {
    ...
    if (IsFloatInstruction(INS_Address(ins))) {
        ...
        case 2:
        case 4:
            HANDLE_APPROXREG(float, false, reg);
            break;
        case 8:
            HANDLE_APPROXREG(double, false, reg);
            break;
        case 10:
            HANDLE_10BYTES_APPROX(reg);
            break;
        case 16: {
            switch (operSize) {
            case 4:
                HANDLE_LARGEREG_APPROX(float, 0, reg - REG_XMM_BASE);
                break;
            case 8:
                HANDLE_LARGEREG_APPROX(double, 0, reg - REG_XMM_BASE);
                break;
            default:
                assert(0 && "handle large reg with large operand size\n");
                break;
            }
        } break;
        case 32: {
            switch (operSize) {
            case 4:
                HANDLE_LARGEREG_APPROX(float, 1, reg - REG_YMM_BASE);
                break;
            case 8:
                HANDLE_LARGEREG_APPROX(double, 1, reg - REG_YMM_BASE);
                break;
            default:
                assert(0 && "handle large reg with large operand size\n");
                break;
        ...
        case 64: {
            switch (operSize) {
            case 4:
                HANDLE_LARGEREG_APPROX(float, 2, reg - REG_ZMM_BASE);
                break;
            case 8:
                HANDLE_LARGEREG_APPROX(double, 2, reg - REG_ZMM_BASE);
                break;
            default:
                assert(0 && "handle large reg with large operand size\n");
                break;
        ...
        default:
            assert(0 && "not recoganized register size for floating instruction!\n");
        }
    } else {
        ...
        switch (regSize) {
        case 1:
            HANDLE_GENERAL(uint8_t);
            break;
        case 2:
            HANDLE_GENERAL(uint16_t);
            break;
        case 4:
            HANDLE_GENERAL(uint32_t);
            break;
        case 8:
            HANDLE_GENERAL(uint64_t);
            break;
        case 16:
            HANDLE_SPECIALREG(2, reg - REG_XMM_BASE);
            break;
        case 32:
            HANDLE_SPECIALREG(4, reg - REG_YMM_BASE);
            break;
        case 64:
            HANDLE_SPECIALREG(8, reg - REG_ZMM_BASE);
            break;
        default:
            assert(0 && "not recoganized register size for integer instruction!\n");
            break;
        }
    }
}
...
struct RedSpyInstrument {
    static __attribute__((always_inline)) void InstrumentReadValueBeforeAndAfterWriting(INS ins, UINT32 memOp, uint32_t opaqueHandle) {
        ...
        if (IsFloatInstruction(INS_Address(ins))) {
            ...
            case 1:
            case 2:
                assert(0 && "memory write floating data with unexptected small size");
            case 4:
                HANDLE_APPROX_CASE(float, 4, readBufferSlotIndex, true);
                break;
            case 8:
                HANDLE_APPROX_CASE(double, 8, readBufferSlotIndex, true);
                break;
            case 10:
                HANDLE_APPROX_CASE(uint8_t, 10, readBufferSlotIndex, true);
                break;
            case 16: {
                switch (operSize) {
                case 4:
                    HANDLE_APPROX_CASE(float, 16, readBufferSlotIndex, true);
                    break;
                case 8:
                    HANDLE_APPROX_CASE(double, 16, readBufferSlotIndex, true);
                    break;
                default:
            ...
            case 32: {
                switch (operSize) {
                case 4:
                    HANDLE_APPROX_CASE(float, 32, readBufferSlotIndex, true);
                    break;
                case 8:
                    HANDLE_APPROX_CASE(double, 32, readBufferSlotIndex, true);
                    break;
                default:
                    assert(0 && "handle large mem write with unexpected operand size\n");
                    break;
            ...
            default:
                assert(0 && "unexpected large memory writes\n");
                break;
            }
        } else {
            switch (refSize) {
            case 1:
                HANDLE_CASE(uint8_t, 1, readBufferSlotIndex, false);
                break;
            case 2:
                HANDLE_CASE(uint16_t, 2, readBufferSlotIndex, false);
                break;
            case 4:
                HANDLE_CASE(uint32_t, 4, readBufferSlotIndex, false);
                break;
            case 8:
                HANDLE_CASE(uint64_t, 8, readBufferSlotIndex, false);
                break;
...
static inline bool INS_IsIgnorable(INS ins) {
    ...
    if (INS_IsCall(ins) || INS_IsSyscall(ins))
        return true;
    if (INS_IsBranch(ins) || INS_IsRDTSC(ins) || INS_IsNop(ins))
        return true;
