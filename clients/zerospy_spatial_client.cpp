...
struct ZeroSpyAnalysis {
    ...
    static __attribute__((always_inline)) VOID CheckNByteValueAfterRead(void* addr, uint32_t opaqueHandle, THREADID threadId) {
        ...
        if (curDataHandle.end_addr - curDataHandle.beg_addr <= SMALL_VAR_THRESHOLD)
            return;
#endif
        uint32_t redbyteNum = getRedNum(addr);
        uint8_t redbyteMap[32] = {0};
        if (redbyteNum)
            getRedMap(addr, redbyteMap);
        else
            memset(redbyteMap, DATA_STATE_NOT_ZERO, 32);
        if (isApprox)
...
struct LoadSpyInstrument {
    static __attribute__((always_inline)) void InstrumentReadValueBeforeAndAfterLoading(INS ins, UINT32 memOp, uint32_t opaqueHandle) {
        ...
        if (IsFloatInstructionAndOkToApproximate(INS_Address(ins))) {
            ...
            case 4:
                HANDLE_CASE(float, 4, true);
                break;
            case 8:
                HANDLE_CASE(double, 8, true);
                break;
            case 10:
                HANDLE_CASE(uint8_t, 10, true);
                break;
            case 16: {
                switch (operSize) {
                case 4:
                    HANDLE_CASE(float, 16, true);
                    break;
                case 8:
                    HANDLE_CASE(double, 16, true);
                    break;
                default:
            ...
            case 32: {
                switch (operSize) {
                case 4:
                    HANDLE_CASE(float, 32, true);
                    break;
                case 8:
                    HANDLE_CASE(double, 32, true);
                    break;
                default:
                    assert(0 && "handle large mem read with unexpected operand size\n");
                    break;
            ...
            switch (refSize) {
            case 1:
                HANDLE_CASE(uint8_t, 1, false);
                break;
            case 2:
                HANDLE_CASE(uint16_t, 2, false);
                break;
            case 4:
                HANDLE_CASE(uint32_t, 4, false);
                break;
            case 8:
                HANDLE_CASE(uint64_t, 8, false);
                break;
...
static void PrintRedundancyPairs(THREADID threadId) {
    ...
    for (vector<ObjRedundancy>::iterator listIt = tmpList.begin(); listIt != tmpList.end(); ++listIt) {
        ...
        for (vector<uint8_t>::iterator addrIt = RedMap[threadId][(*listIt).objID].state.begin(); addrIt != RedMap[threadId][(*listIt).objID].state.end(); ++addrIt) {
            if (*addrIt == DATA_STATE_ONLY_ZERO)
                dfreq++;
            if (*addrIt)
                dread++;
        }
...
static VOID ImageUnload(IMG img, VOID* v) {
    ...
    if (RedMap[threadid].empty() && ApproxRedMap[threadid].empty())
        return;
