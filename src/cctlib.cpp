...
#ifndef MAX_TRACE_NODES
#if defined(TARGET_IA32E)
    // 2^20  IPNODES
#define MAX_TRACE_NODES (1L << 20)
#elif defined(TARGET_IA32)
    // 1M IPNODES
#define MAX_TRACE_NODES (1L << 16)
#else
    "SHOULD NEVER REACH HERE"
#endif
#endif
...
#define UNLOCKED_AND_PREDECESSOR_WAS_WRITER (2)
    ...
    static inline uint32_t GetNextTraceNodeBufferIdx();
    ...
    struct CCT_LIB_GLOBAL_STATE {
        ...
        TraceNode * curPreAllocatedTraceNodeBuffer;
        ...
        uint64_t curPreAllocatedTraceNodeBufferIndex __attribute__((aligned(CACHE_LINE_SIZE))); // align to eliminate any false sharing with other  members
...
#endif
        const ContextHandle_t callerCtxtHndl;
        const ContextHandle_t childCtxtStartIdx;
        const uint32_t traceKey; // max of 2^32 traces allowed
        const uint32_t nSlots;
        TraceNode(ContextHandle_t xcallerCtxtHndl, ContextHandle_t xchildCtxtStartIdx, uint32_t xtraceKey, uint32_t xnSlots):
        callerCtxtHndl(xcallerCtxtHndl), childCtxtStartIdx(xchildCtxtStartIdx), traceKey(xtraceKey), nSlots(xnSlots) {}
        
        void* operator new  (std::size_t count){
            assert(count == sizeof(TraceNode));
            uint32_t idx = GetNextTraceNodeBufferIdx();
            return GLOBAL_STATE.curPreAllocatedTraceNodeBuffer + idx;
        }
    };
    ...
    static inline VOID RestoreSigLongJmpCtxt(THREADID threadId) {
        ...
        UpdateCurTraceOnly(tData,  GET_TRACE_NODE_FROM_CTXT_HANDLE(tData->tlsCurrentCtxtHndl));
...
#define X86_INDIRECT_CALL_SITE_ADDR_FROM_RETURN_ADDR(callsite) (callsite - 2)
    ...
    static TraceNode* FindNearestCallerCoveringIP(ADDRINT exceptionCallerReturnAddrIP, uint32_t* ipSlot, ThreadData* tData) {
        ...
        while(curTrace) {
            ...
            curTrace = GET_TRACE_NODE_FROM_TRACE_IDX(GLOBAL_STATE.preAllocatedContextBuffer[curTrace->callerCtxtHndl].parentTraceNodeIdx);
    ...
    static inline uint32_t GetNextTraceNodeBufferIdx() {
        uint64_t  oldBufIndex = __sync_fetch_and_add(&GLOBAL_STATE.curPreAllocatedTraceNodeBufferIndex, 1);
        if(oldBufIndex + 1  >= MAX_TRACE_NODES) {
            //printf("\nPreallocated IPNodes exhausted. CCTLib couldn't fit your application in its memory. Try a smaller program.\n");
            fprintf(GLOBAL_STATE.CCTLibLogFile, "\nPreallocated TraceNodes exhausted. CCTLib couldn't fit your application in its memory. Try a smaller program.\n");
            PIN_ExitProcess(-1);
        }
        return (uint32_t) oldBufIndex;
    }
    ...
    static inline void CCTLibInitThreadData(ThreadData* const tdata, CONTEXT* ctxt, THREADID threadId) {
        TraceNode* t = new TraceNode(0, GetNextIPVecBuffer(1), GetNextTraceKey(), 1);
        IPNode * ipNode = GET_IPNODE_FROM_CONTEXT_HANDLE(t->childCtxtStartIdx);
        ipNode->parentTraceNodeIdx = GET_TRACE_IDX_FROM_TRACE_NODE(t);
#ifdef USE_SPLAY_TREE
        t->calleeTraceNodes = 0;
#else
    ...
    static struct TraceSplay* splay(struct TraceSplay* root, uint64_t ip) {
    ...
    static inline void InstrumentTraceEntry(uint32_t traceKey, uint32_t numInterestingInstInTrace, THREADID threadId) {
        ...
        if(!tData->tlsInitiatedCall) {
            tData->tlsCurrentCtxtHndl = tData->tlsCurrentTraceNode->callerCtxtHndl;
            tData->tlsCurrentTraceNode = GET_TRACE_NODE_FROM_CTXT_HANDLE(tData->tlsCurrentCtxtHndl);
        } else {
...
#ifdef USE_SPLAY_TREE
        uint64_t myKey = TRACE_ID_AND_CALLER_CONTEXT_64_BIT_KEY(traceKey, tData->tlsCurrentCtxtHndl);
        TraceSplay* found = splay(tData->tlsCurrentTraceNode->calleeTraceNodes, myKey);
        
        // Check if a trace node with traceKey already exists under this context node
        if(found && (myKey == found->key)) {
            tData->tlsCurrentTraceNode->calleeTraceNodes = found;
            ...
            if(numInterestingInstInTrace)
                newChild = new TraceNode(tData->tlsCurrentCtxtHndl, GetNextIPVecBuffer(numInterestingInstInTrace), traceKey, numInterestingInstInTrace /* nSlots*/);
            else
                newChild = new TraceNode(tData->tlsCurrentCtxtHndl, 0 /* budIndex */, traceKey, 0 /* nSlots*/);
            const uint32_t newChildIdx = GET_TRACE_IDX_FROM_TRACE_NODE(newChild);

            newChild->calleeTraceNodes = 0;
            ...
            if(numInterestingInstInTrace) {
                ...
                for(uint32_t i = 0 ; i < numInterestingInstInTrace ; i++) {
                    ipNode[i].parentTraceNodeIdx = newChildIdx;
                }
            } else {
            ...
            tData->tlsCurrentTraceNode->calleeTraceNodes = newNode;
            
            if(!found) {
                newNode->left = NULL;
                newNode->right = NULL;
            } else if(myKey < found->key) {
    ...
    static void VisitAllNodesOfSplayTree(TraceSplay* node, ContextHandle_t whichContext, FILE* const fp) {
        if(node == NULL)
        return;
        
        // visit left
        VisitAllNodesOfSplayTree(node->left, whichContext, fp);
        // process self
        if(whichContext == node->contextHandle)
        SerializeCCTNode(node->value, fp);
        // visit right
        VisitAllNodesOfSplayTree(node->right, whichContext, fp);
    }
    ...
    static void SerializeCCTNode(TraceNode* traceNode, FILE* const fp) {
        ...
        for(uint32_t i = 0 ; i < traceNode->nSlots; i++) {
            ...
            VisitAllNodesOfSplayTree((traceNode->calleeTraceNodes, GET_CONTEXT_HANDLE_FROM_IP_NODE(&traceNode->childCtxtStartIdx[i]), fp));
        }
    }
    ...
    static void VisitAllNodesOfSplayTree(TraceSplay* node, ContextHandle_t whichContext, FILE* const fp) {
        // process self
        if(node->contextHandle == whichContext)
        SerializeCCTNode(node->value, fp);
    ...
    static void SerializeCCTNode(TraceNode* traceNode, FILE* const fp) {
        ...
        // Iterate over all IPNodes
        for(uint32_t i = 0 ; i < traceNode->nSlots; i++) {
            if(traceNode->calleeTraceNodes == NULL) {
                fwrite(&NO_MORE_TRACE_NODES_IN_SPLAY_TREE, sizeof(NO_MORE_TRACE_NODES_IN_SPLAY_TREE), 1, fp);
            } else {
                // Iterate over all decendent TraceNode of traceNode->childCtxtStartIdx[i]
                VisitAllNodesOfSplayTree(traceNode->calleeTraceNodes, traceNode->childCtxtStartIdx + i, fp);
                fwrite(&NO_MORE_TRACE_NODES_IN_SPLAY_TREE, sizeof(NO_MORE_TRACE_NODES_IN_SPLAY_TREE), 1, fp);
            }
        }
    }
    ...
    static TraceNode* DeserializeCCTNode(ContextHandle_t parentCtxtHndl, FILE* const fp) {
        ...
        TraceNode* traceNode = new TraceNode(parentCtxtHndl, serializedTraceNode.childCtxtStartIdx, serializedTraceNode.traceKey, serializedTraceNode.nSlots);
        const uint32_t traceNodeIdx = GET_TRACE_IDX_FROM_TRACE_NODE(traceNode);
        traceNode->calleeTraceNodes = NULL;
        
        
        // Iterate over all IPNodes
        IPNode * ipNode = GET_IPNODE_FROM_CONTEXT_HANDLE(traceNode->childCtxtStartIdx);
        for(uint32_t i = 0 ; i < traceNode->nSlots; i++) {
            ipNode[i].parentTraceNodeIdx = traceNodeIdx;
            ...
            while(1) {
                ...
                TraceSplay* newNode = new TraceSplay();
                newNode->traceKey = childTrace->traceKey;
                newNode->contextHandle = traceNode->childCtxtStartIdx + i;
                newNode->value = childTrace;
                
                
                uint64_t childAndParentContextKey = TRACE_ID_AND_CALLER_CONTEXT_64_BIT_KEY(newNode->traceKey, newNode->contextHandle);
                ...
                if(traceNode->calleeTraceNodes == NULL) {
                    ...
                    uint64_t myKey = TRACE_ID_AND_CALLER_CONTEXT_64_BIT_KEY(childTrace->traceKey, (traceNode->childCtxtStartIdx + i));
                    TraceSplay* found    = splay(traceNode->calleeTraceNodes, myKey);
                    
                    if(childAndParentContextKey < found->key) {
                        newNode->left = found->left;
                        newNode->right = found;
                        found->left = NULL;
                    } else { // addr > addr of found
    ...
    // Visit all nodes of the splay tree of child traces.
    static void ListAllNodesOfSplayTree(TraceSplay* node, ContextHandle_t whichContext, vector<TraceNode*>& childTraces) {
        if(node == NULL)
        return;
        
        // visit left
        ListAllNodesOfSplayTree(node->left, whichContext, childTraces);
        if(whichContext == node->contextHandle)
        childTraces.push_back(node->value);
        // visit right
        ListAllNodesOfSplayTree(node->right, whichContext, childTraces);
    }
    ...
    static void DottifyCCTNode(TraceNode* traceNode,  uint64_t parentDotId, FILE* const fp) {
        ...
        for(uint32_t i = 0 ; i < traceNode->nSlots; i++) {
            ...
            ListAllNodesOfSplayTree(traceNode->calleeTraceNodes, (traceNode->childCtxtStartIdx + i), childTraces);
        }
...
#define SERIALIZED_MODULE_MAP_SUFFIX "/ModuleMap.txt"
    ...
    static inline ADDRINT GetIPFromInfo(ContextHandle_t ctxtHndle) {
        TraceNode* traceNode = GET_TRACE_NODE_FROM_CTXT_HANDLE(ctxtHndle);
    ...
    static inline const string& GetModulePathFromInfo(IPNode* ipNode) {
        TraceNode* traceNode = GET_TRACE_NODE_FROM_TRACE_IDX(ipNode->parentTraceNodeIdx);
...
#define NOT_ROOT_CTX (-1)
    ...
    static VOID PrintFullCallingContext(IPNode* curIPNode) {
        ...
        while(curIPNode && (depth ++ < MAX_CCT_PRINT_DEPTH)) {
            ...
            if((threadCtx = IsARootIPNode(curIPNode)) != NOT_ROOT_CTX) {
                ...
                curIPNode = GET_TRACE_NODE_FROM_TRACE_IDX(curIPNode->parentTraceNodeIdx)->callerCtxtHndl;
            }
        }
    ...
    static VOID GetFullCallingContextInSitu(ContextHandle_t curCtxtHndle, vector<Context>& contextVec) {
        ...
        while(IS_VALID_CONTEXT(curCtxtHndle) && (depth ++ < MAX_CCT_PRINT_DEPTH)) {
            ...
            if((threadCtx = IsARootIPNode(curCtxtHndle)) != NOT_ROOT_CTX) {
                ...
                curCtxtHndle = GET_TRACE_NODE_FROM_CTXT_HANDLE(curCtxtHndle)->callerCtxtHndl;
            }
        }
    ...
    static void InitBuffers() {
        ...
        if(GLOBAL_STATE.preAllocatedContextBuffer == MAP_FAILED){
            perror("Failed mmap preAllocatedContextBuffer; try with lesser MAX_IPNODES");
            PIN_ExitProcess(-1);
        }
        
        GLOBAL_STATE.curPreAllocatedTraceNodeBuffer = (TraceNode*) mmap(0, MAX_TRACE_NODES * sizeof(TraceNode), PROT_WRITE
                                                                | PROT_READ, MAP_NORESERVE | MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
        
        if(GLOBAL_STATE.preAllocatedContextBuffer == MAP_FAILED){
            perror("Failed mmap curPreAllocatedTraceNodeBuffer; try with lesser MAX_TRACE_NODES");
            PIN_ExitProcess(-1);
        }
        ...
        GLOBAL_STATE.curPreAllocatedTraceNodeBufferIndex = 1;
...
#define UNWIND_RESUME_OR_RETHROW "_Unwind_Resume_or_Rethrow"
    ...
    static void BottomUpTraverseHelper(TraceSplay *node, ContextHandle_t whichContext, void (*opFunc) (const THREADID threadid, ContextHandle_t myHandle, ContextHandle_t parentHandle, void **myMetric, void **parentMetric), const THREADID threadid) {
        if (!node) return;
        
        BottomUpTraverseHelper(node->left, whichContext, opFunc, threadid);
        if(node->contextHandle == whichContext)
        BottomUpTraverse(node->value, opFunc, threadid);
        BottomUpTraverseHelper(node->right, whichContext, opFunc, threadid);
    }
    ...
    static void BottomUpTraverse(TraceNode *node, void (*opFunc) (const THREADID threadid, ContextHandle_t myHandle, ContextHandle_t parentHandle, void **myMetric, void **parentMetric), const THREADID threadid) {
        ...
        for(uint32_t i = 0 ; i < node->nSlots; i++) {
            if(node->calleeTraceNodes) {
                // Iterate over all decendent TraceNode of traceNode->childCtxtStartIdx[i]
                BottomUpTraverseHelper(node->calleeTraceNodes, node->childCtxtStartIdx + i, opFunc, threadid);
            }
            // do anything here
    ...
    bool HaveSameCallerPrefix(ContextHandle_t ctxt1, ContextHandle_t ctxt2) {
        if (ctxt1 == ctxt2)
        return true;
        ContextHandle_t t1 = GET_TRACE_NODE_FROM_CTXT_HANDLE(ctxt1)->callerCtxtHndl;
        ContextHandle_t t2 = GET_TRACE_NODE_FROM_CTXT_HANDLE(ctxt2)->callerCtxtHndl;
        return t1 == t2;
    }
...
__sync_val_compare_and_swap(addr, oldval, newval)
    ...
    static IPNode * findMain(TraceSplay* sNode) {
        ...
        IPNode * n = findMain(sNode->value);
        if(n) return n;
        n = findMain(sNode->left);
        if(n) return n;
    ...
    static IPNode * findMain(TraceNode* tNode, TraceNode* rootTraceNode) {
        ...
        return findMain(tNode->calleeTraceNodes);
    }
    ...
    int newCCT_hpcrun_write(THREADID threadid) {
        ...
        if (mainNode) {
            ...
            cctlib = new TraceNode(0 /* dummy callerCtxtHndl*/, GET_TRACE_NODE_FROM_TRACE_IDX(mainNode->parentTraceNodeIdx)->callerCtxtHndl /* dummy root should have 0 ip */, 0/* invalid tracekey*/, 1);
